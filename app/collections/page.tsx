...
export default function CollectionsPage() {
  ...
  useEffect(() => {
    ...
    const loadCollections = () => {
      try {
        ...
        const transformedCollections: Collection[] = storedCollections.map((col: any) => ({
          id: col.id || `collection-${Date.now()}`,
          name: col.name || "Unnamed Collection",
          description: col.description || "",
          image: col.logoImage || col.image || "/placeholder.svg",
          banner: col.bannerImage || col.banner,
          creator: col.creator || {
  ...
  const sortedCollections = [...filteredCollections].sort((a, b) => {
    if (sortBy === "volume") return Number.parseFloat(b.stats.volume) - Number.parseFloat(a.stats.volume)
    if (sortBy === "floor") return Number.parseFloat(b.stats.floorPrice) - Number.parseFloat(a.stats.floorPrice)
    if (sortBy === "items") return b.stats.items - a.stats.items
